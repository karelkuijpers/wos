...
EXPORT WhatFrom, WhoFrom AS STRING
...
* Find subbalance items and add to arrays withbalance Items
	...
	if Empty(aItem)
		oBal:=SQLSelect{"select * from balanceitem",oConn} 
		if oBal:reccount>0
			do while !oBal:EoF
...
METHOD AddSubDep(ParentNum:=0 as int, nCurrentRec:=0 as int,aItem as array,d_dep as array,d_parentdep as array,d_indmaindep as array,d_depname as array,d_acc as array,d_netasset as array,d_netnum as array) as int CLASS BalanceReport
	* Find subdepartments and add to arrays with departments
	...
	if Empty(aItem)
		...
		"FROM `department` d left join account an on(an.accid=d.netasset)",oConn}
		if oDep:reccount>0
			do while !oDep:EoF
...
METHOD BalancePrint(FileInit:="" as string) as void pascal CLASS BalanceReport
	...
	*
	...
	LOCAL PrYr_BalPer, PrYr_BalPrvPer as FLOAT
	LOCAL TopWhatPtr, TopWhoPtr AS INT
	LOCAL nCurRec, Bal_Ptr,Dep_Ptr,kap_num,nCurAcc as int
	...
	local aAcc:={} as array   // array with al used accounts: {accid,accnumber,description}
	...
	* Check and fill requested Departments:
	...
	ELSE
		oDep:=SQLSelect{"select d.ParentDep,d.DEPTMNTNBR,d.Descriptn,d.depid,d.netasset,a.balitemid from department d left join account a on (a.accid=d.netasset) where depid='"+self:WhoFrom+"'",oConn}
		if oDep:reccount>0
	...
	* Add all subdepartments down from self:WhoFrom:  
	aItem:={}
	DO WHILE TRUE
		nCurRec:=self:AddSubDep(Val(self:WhoFrom),nCurRec,aItem,d_dep,d_parentdep,d_indmaindep,d_depname,d_acc,d_netasset,d_netnum)
		IF Empty(nCurrec)
			EXIT
		ENDIF
	ENDDO
	...
	* Fill all requested balance items:
	IF !Empty(self:WhatFrom) 
		oBal:=SQLSelect{"select * from balanceitem where balitemid='"+self:WhatFrom+"'",oConn} 
		IF oBal:reccount>0
			* Add Top item:
	...
	* Add all child balance items:
	...
	aItem:={}
	DO WHILE TRUE
		nCurRec:=self:AddSubBal(Val(self:WhatFrom),nCurRec,aItem,0,r_indmain,r_parentid,r_balid,r_balnbr,r_cat,r_heading,r_footer)
		IF Empty(nCurrec)
			EXIT
		ENDIF
	ENDDO
	...
	* Initialize value arrays with NILvalues:
	...
	rd_salvjper:=ArrayNew(DepCount,BalCount)
	rd_salvjvrg:=ArrayNew(DepCount,BalCount)
	rd_bud:=ArrayNew(DepCount,BalCount)
	rd_budper:=ArrayNew(DepCount,BalCount)
	rd_budytd:=ArrayNew(DepCount,BalCount) 

	oReport:MaxWidth:=Max(oReport:MaxWidth,TotalWidth)
	// First time reinit:
	IF Len(oReport:oPrintJob:aFiFo)<2
		oReport:ReInitPrint(SendToMail)
	ENDIF
	IF !Empty(FileInit)
		AAdd(oReport:oPrintJob:aFiFo,FileInit)
		SELF:iLine := 0
	ENDIF
	...
	* Add balances of accounts to the balance item values: 
	oAcc:=SQLSelect{"select a.accid,a.department, a.description, a.accnumber,a.balitemid,b.category "+;
	"from account a,balanceitem b "+;
	"where a.department in ("+Implode(d_dep,",")+") and a.balitemid in ("+Implode(r_balid,",")+") and a.balitemid=b.balitemid",oConn} 
	oAcc:GoTop() 
	oMbal:=Balances{}
	DO WHILE !oAcc:EOF
// 		IF Empty(oAcc:balitemid)
// 			SELF:Pointer := Pointer{POINTERARROW}
// 			(errorbox{self:OWNER,self:oLan:WGet('Report item empty in account')+':'+oAcc:AccNumber}):show()
// 			RETURN 
// 		ENDIF
		...
		rd_salvjtot[Dep_Ptr,Bal_Ptr]:=if(Empty(rd_salvjtot[Dep_Ptr,Bal_Ptr]),0,rd_salvjtot[Dep_Ptr,Bal_Ptr])+oMBal:vjr_deb-oMBal:vjr_cre
		rd_salvrg[Dep_Ptr,Bal_Ptr]  :=if(Empty(rd_salvrg[Dep_Ptr,Bal_Ptr]),0,rd_salvrg[Dep_Ptr,Bal_Ptr])+oMBal:vorig_deb-oMBal:vorig_cre
		rd_salper[Dep_Ptr,Bal_Ptr]  :=if(Empty(rd_salper[Dep_Ptr,Bal_Ptr]),0,rd_salper[Dep_Ptr,Bal_Ptr])+oMBal:per_deb - oMBal:per_cre
		rd_bud[Dep_Ptr,Bal_Ptr]     :=if(Empty(rd_bud[Dep_Ptr,Bal_Ptr]),0,rd_bud[Dep_Ptr,Bal_Ptr]) +GetPerBudget(self:YEARSTART,self:MONTHSTART,12,@YearGranularity,m_accid)
		rd_budper[Dep_Ptr,Bal_Ptr]  :=if(Empty(rd_budper[Dep_Ptr,Bal_Ptr]),0,rd_budper[Dep_Ptr,Bal_Ptr]) +GetPerBudget(self:YEARSTART,self:MONTHSTART,self:YEAREND*12+MONTHEND-self:YEARSTART*12-self:MONTHSTART+1,@YearGranularity,m_accid)
		rd_budytd[Dep_Ptr,Bal_Ptr]  :=if(Empty(rd_budytd[Dep_Ptr,Bal_Ptr]),0,rd_budytd[Dep_Ptr,Bal_Ptr]) +GetPerBudget(BalYear,BalMonth,self:YEAREND*12+MONTHEND-BalYear*12-BalMonth+1,@YearGranularity,m_accid)
		...
		ENDIF
		AAdd(d_acc[Dep_Ptr],balsoort+Pad(cBalName,25)+Str(oAcc:balitemid,11,0)+Str(oAcc:accid,11,0)+"P")
		...
		* 	bij kapitaal op te tellen (immers nog niet bijgeboekt):
		IF balsoort == '1' .or. balsoort == '2'
			IF self:PrvYearNotClosed
				oMbal:GetBalance(m_accid,oAcc:category,Year(LstYearClosed)*100+Month(LstYearClosed),;
					IF(BalMonth==1,(BalYear-1)*100+12,BalYear*100+BalMonth-1))  && balans t/m vorig balansjaar
				d_PLdeb[Dep_Ptr]:=Round(if(IsNil(d_PLdeb[Dep_Ptr]),0,d_PLdeb[Dep_Ptr]) + oMbal:per_deb,DecAantal)
				d_PLcre[Dep_Ptr]:=Round(if(IsNil(d_PLcre[Dep_Ptr]),0,d_PLcre[Dep_Ptr]) + oMbal:per_cre,DecAantal)
				IF self:YearBeforePrvNotClosed
					* Indien jaar daar weer voor ook niet afgesloten, dan V&W over die periode bepalen en
					* bij kapitaal van voorgaande periode tellen.
					oMbal:GetBalance(m_accid,oAcc:category,Year(LstYearClosed)*100+Month(LstYearClosed),;
						IF(BalMonth==1,(BalYear-2)*100+12,(BalYear-1)*100+BalMonth-1))  && balans t/m voorvorig balansjaar
					d_PrfLssPrYr[Dep_Ptr]:=Round(if(IsNil(d_PrfLssPrYr[Dep_Ptr]),0,d_PrfLssPrYr[Dep_Ptr]) + oMbal:per_deb- oMbal:per_cre,DecAantal)
					*				VWvj:=VWvj - oMbal:per_cre + oMbal:per_deb
				ENDIF
			ENDIF
		ENDIF
		...
		rd_salvjvrg[Dep_Ptr,Bal_Ptr]:=if(Empty(rd_salvjvrg[Dep_Ptr,Bal_Ptr]),0,rd_salvjvrg[Dep_Ptr,Bal_Ptr])+oMBal:vorig_deb - oMBal:vorig_cre
		rd_salvjper[Dep_Ptr,Bal_Ptr]:=if(Empty(rd_salvjper[Dep_Ptr,Bal_Ptr]),0,rd_salvjper[Dep_Ptr,Bal_Ptr])  +oMBal:per_deb - oMBal:per_cre
		if oAcc:recno%50=1
			nCurAcc:=nCurAcc
		endif
		oAcc:skip()
		...
		FOR dep_ptr:=1 TO DepCount
			* Determine pointer to balance item for capital: (can be empty)
			...
			IF !IsNil(d_PLdeb[Dep_Ptr]).and.!Empty(kap_num)
				...
				ENDIF
				IF IsNil(rd_salvjvrg[dep_ptr,kap_num])
					rd_salvjvrg[dep_ptr,kap_num]:=0
				ENDIF
				IF IsNil(rd_salvjper[dep_ptr,kap_num])
					rd_salvjper[dep_ptr,kap_num]:=0
				ENDIF
	...
	*
	...
	SELF:SubDepartment(1,0,d_netnum,d_indmaindep,d_depname,d_parentDep,d_dep,;
		r_cat,r_balpryrtot,r_balPrYrPrvPer,r_balPrYrPer,r_balPrvPer,r_balPer,r_indmain,r_parentid,;
		r_heading,r_footer,r_balnbr,r_balid,r_bud,r_budper,r_budytd,;
		rd_salvjtot,rd_salvjvrg,rd_salvjper,rd_salvrg,rd_salper,rd_bud,rd_budper,rd_budytd,@iLine,@iPage)
		...
		* Enforce order on department name:
		...
		FOR i:= 1 TO DepCount
			...
			FOR rektel=1 to Len(accnts)
				IF SubStr(accnts[rektel],27,11) # m_balId
					IF Totalize
						IF aant_gev>1
							...
							self:prbedragen(m_cat,rd_salvjtot[Dep_Ptr,Bal_Ptr],;
								rd_salvjvrg[Dep_Ptr,Bal_Ptr],rd_salvjper[Dep_Ptr,Bal_Ptr],;
					...
					IF SubStr(accnts[rektel],1,1) # mType
						...
						oReport:PrintLine(@iLine,@iPage,kop,hfdkop,0)
					ENDIF
				...
				PrYr_BalPer:=oMBal:per_deb-oMBal:per_cre
				PrYr_BalPrvPer:=oMBal:vorig_deb-oMBal:vorig_cre
				oMbal:GetBalance(m_accid,m_cat,self:YEARSTART*100+self:MONTHSTART,self:YEAREND*100+MonthEnd)  && bal jaar
				IF !lCondense.or.!PrYr_BalPer=0.or.!PrYr_BalPrvPer=0.or.!(oMbal:vorig_deb=oMbal:vorig_cre).or.!(oMbal:per_deb=oMbal:per_cre)
					//SELF:StatusMessage("Printing details:"+oAcc:ACCNUMBER)
					IF Empty(aant_gev)
						Totalize:=true
						oReport:PrintLine(@iLine,@iPage,' ',hfdkop,1)
						*					oReport:PrintLine(@iLine,@iPage,r_footer[Dep_Ptr,Bal_Ptr],hfdkop,0)
						oReport:PrintLine(@iLine,@iPage,r_heading[Bal_Ptr],hfdkop,0)
					ENDIF
					nCurAcc:=AScan(aAcc,{|x|x[1]=Val(m_accid)} )
					if nCurAcc>0
					self:prbedragen(m_cat,oMbal:vjr_deb-oMbal:vjr_cre,PrYr_BalPrvPer,PrYr_BalPer,;
						oMBal:vorig_deb-oMBal:vorig_cre,oMBal:per_deb-oMBal:per_cre,;
						GetPerBudget(self:YEARSTART,self:MONTHSTART,12,@YearGranularity,m_accid),;
						GetPerBudget(self:YEARSTART,self:MONTHSTART,self:YEAREND*12+MONTHEND-self:YEARSTART*12-self:MONTHSTART+1,@YearGranularity,m_accid),;
						GetPerBudget(self:YEARSTART,self:MONTHSTART,self:YEAREND*12+MONTHEND-BalYear*12-BalMonth+1,@YearGranularity,m_accid),;
						1,if(self:Numbers,aAcc[nCurAcc,2]+self:TAB,"")+aAcc[nCurAcc,3],r_heading,0.00,0.00,@iLine,@iPage) 
					endif
					++aant_gev
					/*		ELSE
...
METHOD BalButton( ) CLASS BalanceReport
	...
	ENDIF
	(BalanceItemExplorer{self:Owner,"Balance Item",WhatFrom,self,cBalValue}):show()
RETURN NIL
...
MONTHSTART:=aBal[2]
MONTHEND:=aBal[4]
oDCBalYears:CurrentItemNo:=1
WhatDetails:=TRUE
WhoDetails:=TRUE
mBalNumber:="0: Balance Structure"
...
METHOD prbedragen(pr_soort,pr_salvjtot,pr_salvjvrg,pr_salvjper,pr_salvrg,pr_sal,;
pr_bud,pr_budper,pr_budytd,pr_level,pr_oms,r_koptekst,pr_inc,pr_exp,iLine,iPage) CLASS BalanceReport
* Afdrukken van bedragen per rubriek of rekening
LOCAL regel as STRING
LOCAL mvjsaltot,mvrgsal,msal,mvjvrgsal,mvjsalper,vjtm_perc as FLOAT
...
Default(@pr_salvjvrg,0)
Default(@pr_salvjper,0)
Default(@pr_salvrg,0)
Default(@pr_sal,0)
Default(@pr_bud,0)
Default(@pr_budper,0)
...
HeadingCache:={}
	...
	IF pr_soort=='BA'.or.pr_soort=='PA'
      ...
      mvrgsal:=-pr_salvrg
      msal:=-pr_sal
      mvjvrgsal:=-pr_salvjvrg
      mvjsalper:=-pr_salvjper
	ELSE
      mvjsaltot:=pr_salvjtot
      mvrgsal:=pr_salvrg
      msal:=pr_sal
...
// 	regel:=iif(self:Numbers,self:Tab,"")+iif(self:lXls,iif(pr_level>1,Replicate(self:Tab,pr_level-1),'')+pr_oms+iif(levelrest>0,Replicate(self:Tab,levelrest),''),Pad(Space(pr_level*2)+pr_oms,BalColWidth))+self:Tab
	...
	IF pr_soort=='KO'.or.pr_soort=='BA'
		if !self:SimpleDepStmnt
			IF mvjsaltot # 0
				vjtm_perc:=(mvjvrgsal+mvjsalper)*100/mvjsaltot
	...
	ELSEIF pr_soort=='SD'
		regel:=regel+Str(mvjsalper,9,DecAantal)+self:TAB+Str(mvjsaltot,11,DecAantal)+self:TAB+;
	...
	IF pr_soort=='PA'.or.pr_soort=='AK'
		regel:=regel+self:TAB+Str(mvjsalper,11,decaantal)+'  '+self:TAB+Str(msal,11,decaantal)
	ENDIF
	IF pr_soort=='KO'.or.pr_soort=='BA'
		mtmsal:=mvrgsal+msal
		vrbr_perc:=0
...
//          per_perc:=msal*1200/(pr_bud*(CurEnd-CurSt+1))
 //         tm_perc:=mtmsal*1200/(pr_bud*(CurEnd-BalSt+1))
      ...
      ENDIF
      regel:=regel+iif(self:SimpleDepStmnt,'',self:TAB+Str(mvjvrgsal+mvjsalper,11,decaantal)+space(2)+self:TAB+Str(msal,11,decaantal)+;
...
r_cat as array,r_balpryrtot as array,r_balPrYrPrvPer as array,r_balPrYrPer as array,r_balPrvPer as array,r_balPer as array,r_indmain as array,r_parentid as array,r_heading as array,r_footer as array,r_bud as array,r_budper as array,r_budytd as array,r_balid as array,;
rd_salvjtot as array,rd_salvjvrg as array,rd_salvjper as array,rd_salvrg as array,rd_salper as array,rd_bud as array,rd_budper as array,rd_budytd as array,iLine ref int,iPage ref int) as void pascal CLASS BalanceReport
...
FOR TopWhatPtr:=1 to self:BalCount
	...
	AAdd(r_balPrYrPrvPer,rd_salvjvrg[p_depptr, TopWhatPtr])
	AAdd(r_balPrYrPer,rd_salvjper[p_depptr, TopWhatPtr])
...
IF Empty(self:WhatFrom)  // virtual Top
	...
	DO WHILE TopWhatPtr<self:BalCount
		...
		r_balPrYrPrvPer,r_balPrYrPer,r_balPrvPer,r_balPer,r_bud,r_budper,r_budytd,r_cat,r_balid,d_netnum,d_depname,aTot,aTotprv,@iLine,@iPage)
	ENDDO
ELSE
...
METHOD RegBalance(myNum) CLASS BalanceReport 
	...
	WhatFrom:=myNum
	IF Empty(WhatFrom)
		cCurBal:="0: Balance Structure"
		mBalNumber:=cCurBal
		oDCmBalNumber:TextValue:=cCurBal
	ELSE
		oBal:=SQLSelect{"select number,heading from BalanceItem where balitemid='"+WhatFrom+"'",oConn}
		IF oBal:RecCount>0
			cCurBal:=AllTrim(oBal:number)+":"+oBal:Heading
			mBalNumber:=cCurBal
			oDCmBalNumber:TextValue:=cCurBal
		ENDIF
	ENDIF
...
METHOD RegDepartment(myNum,myItemName) CLASS BalanceReport
	...
	ELSE
		oDep:=SQLSelect{"select deptmntnbr,descriptn from department where depid='"+WhoFrom+"'",oConn}
		if oDep:RecCount>0  
...
r_balPrYrPrvPer as array,r_balPrYrPer as array,r_balPrvPer as array,r_balPer as array,r_bud as array,r_budper as array,r_budytd as array,r_cat as array,r_balid as array,d_netnum as array,d_depname as array,aTot as array,aTotprv as array,iLine ref int,iPage ref int) as int CLASS BalanceReport
...
*			oReport:PrintLine(@iLine,@iPage,Space(level*2)+r_heading[bal_ptr],hfdkop,0)
		...
		* test if accounts record directly to this level
		IF !Empty( r_balpryrtot[Bal_Ptr]).or.;
		 !Empty(r_balPrYrPrvPer[Bal_Ptr]).or.;
		 !Empty(r_balPrYrPer[Bal_Ptr]).or.;
		 ...
		 !Empty(r_balPer[Bal_Ptr])
			* Print directly recorded amounts:
			...
			if (r_cat[bal_ptr]=="BA".or.r_cat[bal_ptr]=="KO") .or. !self:SimpleDepStmnt      // skip balance items in case of simple report
				self:prbedragen(Upper(r_cat[bal_ptr]),r_balpryrtot[bal_ptr],;
				r_balPrYrPrvPer[bal_ptr],r_balPrYrPer[bal_ptr],;
	...
	DO WHILE TRUE
		...
		ELSE
			...
			IF !IsNil(r_balPer[SubBalPtr])
				...
				r_balPrYrPrvPer[bal_ptr]:=if(Empty(r_balPrYrPrvPer[bal_ptr]),0.00,r_balPrYrPrvPer[bal_ptr])+r_balPrYrPrvPer[SubBalPtr]
				r_balPrYrPer[bal_ptr]:=if(Empty(r_balPrYrPer[bal_ptr]),0.00,r_balPrYrPer[bal_ptr])+r_balPrYrPer[SubBalPtr]
...
IF (WhatDetails)
	...
	if (m_soort=="BA".or.m_soort=="KO") .or. !self:SimpleDepStmnt      // skip balance items in case of simple report
		...
		IF	!r_indmain[Bal_Ptr] .or.	TotalFound>1
			...
			ENDIF
			self:prbedragen(m_soort,r_balpryrtot[Bal_Ptr],;
			r_balPrYrPrvPer[Bal_Ptr],r_balPrYrPer[Bal_Ptr],;
			r_balPrvPer[Bal_Ptr],r_balPer[Bal_Ptr],r_bud[Bal_Ptr],r_budper[Bal_Ptr],r_budytd[Bal_Ptr],level,pr_oms,r_heading,0.00,0.00,@iLine,@iPage)
		ENDIF
	...
	IF level==0 .and. (!Empty(SELF:WhatFrom).or.bal_ptr>1) && bij 2e keer "beneden": surplus BA of AK
		...
		IF !SDgevonden
			...
			IF !IsNil(r_balpryrtot[1]) .and.!IsNil(r_balPer[1]) .and.!IsNil(r_balPrvPer[1])     //toegevoegd: +r_balPrvPer[1] vanwege startmaand > 1
				...
				ELSE
					surplusvj:=r_balPrYrPer[1]+r_balPrvPer[1]
					surplus:=r_balPer[1]+r_balPrvPer[1]     //toegevoegd: +r_balPrvPer[1] vanwege startmaand > 1
				ENDIF
			//IF clbalvj#surplusvj .or. clbal#surplus
				SDgevonden:=TRUE
...
//				clbalvj:=-(r_balpryrtot[kap_num]-surplusvj)
			...
			IF !SELF:showopeningclosingfund
				...
				self:prbedragen("SD",clbalvj,r_balPrYrPrvPer[Bal_Ptr],surplusvj,;
...
r_cat as array,r_balpryrtot as array,r_balPrYrPrvPer as array,r_balPrYrPer as array,r_balPrvPer as array,r_balPer as array,r_indmain as array,r_parentid as array,;
...
rd_salvjtot as array,rd_salvjvrg as array,rd_salvjper as array,rd_salvrg as array,rd_salper as array,rd_bud as array,rd_budper as array,rd_budytd as array,iLine ref int,iPage ref int) as int CLASS BalanceReport
...
IF d_indmaindep[p_depptr]
	IF WhoDetails
		IF !WhatDetails
			oReport:PrintLine(@iLine,@iPage,Space(level*2)+d_depname[p_depptr],hfdkop,0)
		ENDIF
		* test if accounts record directly to this department:
		IF AScan( rd_salvjtot[p_depptr], {|x| !IsNil(x)})>0 .or.;
		AScan( rd_salvjvrg[p_depptr], {|x| !IsNil(x)})>0 .or.;
		AScan( rd_salvjper[p_depptr], {|x| !IsNil(x)})>0 .or.;
		...
		AScan( rd_salper[p_depptr], {|x| !IsNil(x)})>0
			* Process directly recorded amounts:
			...
			r_cat,r_balpryrtot,r_balPrYrPrvPer,r_balPrYrPer,r_balPrvPer,r_balPer,r_indmain,r_parentid,r_heading,r_footer,r_bud,r_budper,r_budytd,r_balid,;
			rd_salvjtot,rd_salvjvrg,rd_salvjper,rd_salvrg,rd_salper,rd_bud,rd_budper,rd_budytd,@iLine,@iPage)
			IF !WhatDetails
				TotalFound:=1
			ENDIF
	...
	DO WHILE TRUE
		...
		ELSE
			...
			* consolidate values of subdepartment into itself:
			FOR TopWhatPtr:= 1 TO BalCount
				IF !IsNil(rd_salvjper[subDepPtr,TopWhatPtr])
					...
					ENDIF
					IF !IsNil(rd_salvjvrg[subDepPtr,TopWhatPtr])
						rd_salvjvrg[p_depptr,TopWhatPtr]:=if(Empty(rd_salvjvrg[p_depptr,TopWhatPtr]),0.00,;
						rd_salvjvrg[p_depptr,TopWhatPtr])+rd_salvjvrg[subDepPtr,TopWhatPtr]
					ENDIF
					IF !IsNil(rd_salvjper[subDepPtr,TopWhatPtr])
						rd_salvjper[p_depptr,TopWhatPtr]:=if(Empty(rd_salvjper[p_depptr,TopWhatPtr]),0.00,;
						rd_salvjper[p_depptr,TopWhatPtr])+rd_salvjper[subDepPtr,TopWhatPtr]
					ENDIF
	...
	ENDDO
ENDIF
IF WhoDetails.or.p_depptr==1
	IF WhoDetails .and. !Whatdetails
...
BEGIN
	...
	CONTROL	"Cancel", DEPTREPORT_CANCELBUTTON, "Button", WS_TABSTOP|WS_CHILD, 324, 192, 53, 12
	CONTROL	"", DEPTREPORT_SUBSET, "ListBox", LBS_DISABLENOSCROLL|LBS_NOINTEGRALHEIGHT|LBS_MULTIPLESEL|LBS_SORT|LBS_NOTIFY|WS_TABSTOP|WS_CHILD|WS_BORDER|WS_VSCROLL, 248, 18, 125, 166, WS_EX_CLIENTEDGE
...
METHOD ButtonClick(oControlEvent) CLASS DeptReport
	...
	//Put your changes here
	naam:=oControl:Name
	IF !naam="SEPARATEFILE" .and. naam#"PRINTALL"
		RETURN
	ENDIF
	IF ValidateControls( SELF, SELF:AControls )
		IF naam == "SEPARATEFILEMAIL"
			IF !IsMAPIAvailable()
		...
		ELSE
			oMbal:GetBalYear(Val(SubStr(BalYears,1,4)),Val(SubStr(BalYears,5,2)))
		   ...
		   oReport := PrintDialog{,self:oLan:WGet("Dept statements")+" "+Str(self:YEAREND,4,0)+"-"+StrZero(self:MONTHEND,2),,137,DMORIENT_LANDSCAPE}
			IF naam = "SEPARATEFILE"
				oReport:OkButton("File",TRUE)
			ELSE
				oReport:Show()
			ENDIF
			IF .not.oReport:lPrintOk
...
LOCAL aDep:={} AS ARRAY
LOCAL mDep,jaarmaand, mailname AS STRING
...
LOCAL aASS,aAcc AS ARRAY
LOCAL FileInit, cDepName as STRING 
local oBal as BalanceItem 
local nCurFifo as int
...
IF oPers == NULL_OBJECT
	...
	ENDIF
ENDIF
jaarmaand:=Str(YEARSTART,4)+' '+maand[MONTHSTART]+" "+oLan:Get('up incl')+Str(YEAREND,4)+' '+maand[MONTHEND]
//TotalWidth=137
...
ENDIF
IF SendingMethod=="SeperateFileMail"
	oSelpers:=SelPers{,,oPers,oDep}
	oSelpers:AnalyseTxt(oEMLFrm:Template,@ind_openpost,@ind_gift,@ind_naw,@ind_herh,1)
ENDIF
...
IF oDep == NULL_OBJECT
	...
	ENDIF
ENDIF
IF !oDep:OrderInfo(DBOI_NAME)=="DEPNBR"
...
oBalReport:BalYears:=BalYears
oBalReport:lCondense:=TRUE
oBalReport:ind_accstmnt:=FALSE
oBalReport:ind_toel:=FALSE
oBalReport:WhatDetails:=TRUE
oBalReport:WhoDetails:=FALSE
oBalReport:WhatFrom:=Space(11)
oBalReport:MONTHEND:=SELF:MONTHEND
oBalReport:MONTHSTART:=SELF:MONTHSTART
oBalReport:oReport:=oReport
oBalReport:showopeningclosingfund:=true 
oBalReport:SimpleDepStmnt:= self:oDCSimpleDepStmnt:Checked
// if self:oDCSimpleDepStmnt:Checked
// 	// determin income/expense top item:
// 	oBal:=BalanceItem{,DBSHARED,DBREADONLY}
// 	oBal:SetOrder("SUBITEM")
// 	oBal:SetFilter({||oBal:category="BA" .or. oBal:category="KO"})
// 	if oBal:Seek(Space(11))
// 		oBalReport:WhatFrom:=oBal:balitemid
// 	endif
// 	oBal:Close()
// endif
IF MONTHSTART # oMbal:MONTHSTART
//	oBalReport:showopeningclosingfund:=FALSE
ENDIF
IF SendingMethod="SeperateFile"
	oBalReport:SendToMail:=true
else
	oBalReport:SendToMail:=false	
ENDIF
oBalReport:BeginReport:=SELF:BeginReport	
//DO WHILE oDep:DEPTMNTNBR <= ToDep .and. !oDep:EOF
DO WHILE !oDep:EOF
	mdep:=oDep:DepId
	IF !Empty(aDep)
		IF AScan(aDep,mdep)=0
			oDep:Skip()
			LOOP
		ENDIF
	ENDIF
	cDepName:=CleanFileName(AllTrim(StrTran(oDep:DESCRIPTN,"."," ")))
	...
	oBalReport:WhoFrom:=mdep
	oBalReport:iPage:=nPage
	* Insert departmentname if print to seperate file:
	FileInit:=""
	IF lPrintFile .and.SendingMethod="SeperateFile"
		FileInit:=MEMBER_START+cDepName
	ENDIF

	oBalReport:BalancePrint(FileInit)
	self:STATUSMESSAGE("Printing Accountstatements for "+cDepName+", please wait...")

   	IF oTransMonth==NULL_OBJECT
   		oTransMonth:=AccountStatements{60}
   	ENDIF
	oTransMonth:oReport:=SELF:oReport
	oTransMonth:BeginReport:=self:BeginReport
	...
	oAcc:Seek(mdep)
	nPage:=oBalReport:iPage
	aAcc:={}
	nRow:=0
	DO WHILE !oAcc:Eof .and. oAcc:Department==mdep
		AAdd(aAcc,{oAcc:ACCNUMBER,AllTrim(oAcc:Description)})
		oAcc:Skip()
	ENDDO
	ASort(aAcc,,,{|x,y|x[1]<=y[1]})

	FOR i=1 TO Len(aAcc)
		self:STATUSMESSAGE("Printing Accountstatements for "+cDepName+" - "+aAcc[i,2]+", please wait...")
		nRow:=0  && forceer bladskip
		oTransMonth:MonthPrint(aAcc[i,1],aAcc[i,1],YEARSTART,MonthStart,YEAREND,MonthEnd,@nRow,@nPage,addHeading,self:oLan)
	NEXT
	
	*	If associated accounts for this department, print also corresponding accountstatements for this month:
	IF !Empty(oDep:ASSACC1).or.!Empty(oDep:ASSACC2).or.!Empty(oDep:ASSACC3)
		nRow:=0  && forceer bladskip
		IF !oAcc:OrderInfo(DBOI_NAME)=="accid"
			oAcc:setOrder("accid")
		ENDIF
		aASS:={oDep:ASSACC1,oDep:ASSACC2,oDep:ASSACC3}
		FOR i:=1 TO 3
	   		IF !Empty(aASS[i])
				IF !oAcc:OrderInfo(DBOI_NAME)=="accid"
					oAcc:setOrder("accid")
				ENDIF
				oAcc:Seek(aASS[i])
				SELF:Statusmessage("Printing Associated Accountstatements for "+cDepName+" - "+AllTrim(oAcc:description)+", please wait...")
				oTransMonth:MonthPrint(oAcc:ACCNUMBER,oAcc:ACCNUMBER,YEARSTART,MonthStart,YEAREND,MonthEnd,@nRow,@nPage,addHeading+Space(1)+oLan:Get("Associated",,"@!"),self:oLan)
				nRow:=0  && forceer bladskip
	   		ENDIF
	   	NEXT
	ENDIF
	...
	self:GiftsPrint(aAcc,@nRow,@nPage,addHeading)
	if	nCurFiFo==Len(self:oReport:oPrintJob:aFIFO)
		// nothing printed:
		self:oReport:PrintLine(@nRow,@nPage,self:oLan:RGet("No financial activity in given period"),{self:oLan:RGet("Department")+Space(1)+cDepName+":"+jaarmaand},2)
...
*	Proces e-mail:
	IF lPrintFile.and. SendingMethod=="SeperateFileMail"
		IF !Empty(oDep:persid)
			AAdd(aMailDepartment,{AllTrim(StrTran(oDep:DESCRIPTN,"."," ")),oDep:persid,oDep:DepId})
		ENDIF
		IF !Empty(oDep:persid2)
			AAdd(aMailDepartment,{AllTrim(StrTran(oDep:DESCRIPTN,"."," ")),oDep:persid2,oDep:DepId})
		ENDIF
	ENDIF
	oDep:Skip()
ENDDO
...
IF IsString(cFileName)
	...
	IF SendingMethod=="SeperateFileMail" 
		...
		FOR i:=1 TO Len(aMailDepartment)
			...
			oPers:Seek(aMailDepartment[i,2])
			IF oPers:EoF
				MessageBox( 0 , "Contact person for "+aMailDepartment[i,1]+" not found; no report send" , "Problem" , MB_ICONEXCLAMATION )
			ELSE
				* Resolve department responsible person name:
				oRecip := oMapi:ResolveName( AllTrim(oPers:lastname),oPers:persid,oPers:lastname,oPers:email)

				IF oRecip != NULL_OBJECT
					IF !Empty(oEMLFrm:Template) 
						oDep:Seek(aMailDepartment[i,3])
						brieftxt:=oSelpers:FillText(oEMLFrm:Template,1,ind_openpost,ind_gift,ind_naw,ind_herh,60)
					ELSE
						brieftxt:=""
					ENDIF
					oFileSpec:=FileSpec{cFileName}
					oFileSpec:FileName:=AllTrim(oFileSpec:FileName)+" "+aMailDepartment[i,1]
					oMAPI:SendDocument( oFileSpec,oRecip,oRecip2,oLan:Get('Department Statements',,"@!")+": "+jaarmaand,brieftxt)
				ENDIF
			ENDIF
		NEXT
		oMAPI:Close()
...
METHOD GetBalYears() CLASS DeptReport
	// get array with balance years
	RETURN (oMBal:GetBalYears())
METHOD GiftsPrint(aAcc,nRow,nPage,addHeading) CLASS DeptReport
	// printing of the gifts of one department
	* aAcc: array with accounts of the department
...
LOCAL m71_deb,m71_cre,m71_gifthb,m71_giftrst as FLOAT
LOCAL  me_hbn, ocln,mndtxt,hlptxt, g_na1 as STRING
LOCAL jaarmaand,me_cln,m_accid as STRING
LOCAL startmaand as date
LOCAL Inhdrij as int
LOCAL fBed as FLOAT, iMaand as int
...
* ARRAY aGeverLst met geverlijstbedragen met per regel:
* 1 - BED
* 2 - persid
* 3 - description
* 4 - GC
...
LOCAL ogAcc as Account
Default(@addHeading,null_string)
...
IF oPPcd==null_object
	oPPcd:=PPCODES{,DBSHARED,DBREADONLY}
ENDIF
IF !oPPcd:Used
	self:EndWindow()
ENDIF
lSuccess:=oPPcd:SetOrder("PPCODE")
IF oTrans == null_object
	oTrans:=TransHistory{,,"balitemid",true}
	IF !oTrans:Used
		oTrans := null_object
		self:EndWindow()
	ENDIF
...
IF oPers == null_object
	...
	ENDIF
ENDIF
IF ogAcc == null_object
	ogAcc := Account{,DBSHARED,DBREADONLY}
	IF !ogAcc:Used
		self:EndWindow()
	ENDIF
ENDIF
ogAcc:SetOrder("ACCNTNBR")

startmaand:=SToD(Str(YEAREND,4)+'0101')
self:Pointer := Pointer{POINTERHOURGLASS}
self:STATUSMESSAGE(self:oLan:WGet("Collecting data for the report, please wait")+"...")
nRow := 0

* Adapt Top of TransHistory to oldest year:
oTrans:ResetHistory(YEAREND,1,YEAREND,MONTHEND)

ogAcc:SetFilter("GiftAlwd")
aOPP:={}
aGeverLst:={}
for i:=1 to 4
	for j:=1 to 12
...
*	Vul bestand geverlst met desbetreffende mutaties en bepaal totalen:

*	Search first transaction of m_accid in first month:
	oTrans:BeginCondition:=m_accid+Str(YEAREND,4,0)+"01"
	oTrans:EndCondition:='accid=="'+m_accid+'"'
	* Search first occurence of m_accid within given YEAREND:
	oTrans:GoTop()
	IF !oTrans:accid==m_accid
		oTrans:Seek(m_accid)
	ENDIF
	* Search until required startmonth found:
	DO WHILE oTrans:accid==m_accid .and.oTrans:dat<startmaand .and.!oTrans:EOF
		oTrans:Skip()
	ENDDO

	FOR nMonth:=1 to MONTHEND step 1
		IF MONTHEND=nMonth
			store 0 to m71_gifthb,m71_giftrst
			lFirstInMonth:=true
        ENDIF
		DO WHILE oTrans:accid=m_accid.and.Year(oTrans:dat)=YEAREND.and.Month(oTrans:dat)<=nMonth ;
		.and..not. oTrans:EOF


*			Afhandelen giften:
// 			IF .not. Empty(oTrans:persid) .and. oTrans:deb#oTrans:cre .and.!oTrans:FROMRPP
			IF .not. Empty(oTrans:persid) .and. oTrans:deb#oTrans:cre 
				fBed:=  oTrans:cre - oTrans:deb
				iMaand:= Month(oTrans:dat)
				AAdd(aGeverLst,{fBed,oTrans:persid,AllTrim(oTrans:description),;
				IF(ADMIN="WO".or.Admin="HO",Upper(oTrans:gc),""),iMaand,;
				"","","","","","",""})
*				Werk totalen per inhoudingscode en maand bij:
				aAssmntAmount[1,iMaand]:=aAssmntAmount[1,iMaand] + fBed   && total
				IF Upper(oTrans:gc) = "PF"
					Inhdrij := 3
				ELSEIF Upper(oTrans:gc) = "MG"
					Inhdrij := 4
				ELSE
					Inhdrij := 2         && default = assessable
				ENDIF
				aAssmntAmount[Inhdrij,iMaand]:=aAssmntAmount[Inhdrij,iMaand]+fBed
			ENDIF
			oTrans:Skip()
		ENDDO
	NEXT
...
for i:=1 to 4
	for j:=1 to 12
		oGiftRpt:aAssmntAmount[i,j]:=aAssmntAmount[i,j]
	next
next
self:oGiftRpt:GiftsOverview(YEAREND,MonthEnd,Footnotes, aGeverLst,oReport,oDep:DEPTMNTNBR+' '+oDep:DESCRIPTN,@nRow,@nPage,addHeading)
...
METHOD PostInit(oWindow,iCtlID,oServer,uExtra) CLASS DeptReport
	//Put your PostInit additions here
self:SetTexts()
oMbal:GetBalYear(Year(Today()),Month(Today()))
MONTHSTART:=Round(oMBal:MONTHSTART,0)
MONTHEND:=Round(oMBal:MONTHEND,0)
oDCBalYears:CurrentItemNo:=1
mDepartment:="0:"+sEntity+" "+sLand
...
ENDIF
oDep:SetOrder("DepNbr")
// oDep:SetFilter("EvalFltr(DEPID)")
SELF:oDCSubSet:Server:=oDep
oDep:GoTop()
IF !oDep:EoF
...
METHOD PreInit(oWindow,iCtlID,oServer,uExtra) CLASS DeptReport
	...
	RETURN NIL
METHOD RegDepartment(MyNum,ItemName) CLASS DeptReport
	LOCAL depnr:="", deptxt:="",deporder  AS STRING
	...
	ELSE
		IF oDep==NULL_OBJECT
			...
			ENDIF
		ENDIF
   		deporder:=oDep:OrderInfo(DBOI_NAME)
		IF !deporder=="DEPID"
			oDep:SetOrder("DEPID")
		ENDIF
		IF oDep:seek(MyNum)
			depnr:=LTrimZero(oDep:DEPID)
			deptxt:=AllTrim(oDep:DESCRIPTN)
		ENDIF
		oDep:SetOrder(deporder)
	ENDIF	
	IF ItemName == "From Department"
		...
		self:oDCSubSet:DepNbrStart:=depnr
	ENDIF
		...
		*	Print statement report
		...
		FOR nMonth:=1 to ReportMonth step 1
			...
			IF ReportMonth=nMonth.and.self:SkipInactive .and. !lTransFound
				*  Overslaan van medewerkers zonder mutaties:
			...
			if !self:Memberstmnt=="NOST"
				* processing end of month:
				IF	ReportMonth=nMonth.or.self:Memberstmnt=="ALLST"
					*	Afdrukken slotregels	rekeningoverzicht:
...
class YearClosing inherit DataWindowMine 
  ...
  //{{%UC%}} USER CODE STARTS HERE (do NOT remove this line)
	...
	PROTECT oAcc AS Account
	PROTECT oDep AS Department
	PROTECT oMember AS Members
	PROTECT oMBal as MBalance
	PROTECT oAccBal AS AccountBalanceYear
	PROTECT oBalYear AS BalanceYear
	PROTECT oSys AS Sysparms
	PROTECT oDue AS DueAmount
...
RESOURCE YearClosing DIALOGEX  14, 13, 279, 88
STYLE	WS_CHILD
FONT	8, "MS Shell Dlg"
BEGIN
	CONTROL	"OK", YEARCLOSING_OKBUTTON, "Button", BS_DEFPUSHBUTTON|WS_TABSTOP|WS_CHILD, 206, 14, 53, 12
	CONTROL	"Cancel", YEARCLOSING_CANCELBUTTON, "Button", WS_TABSTOP|WS_CHILD, 205, 36, 53, 13
...
IF !oMBal==NULL_OBJECT
	...
	oMBal:=NULL_OBJECT
ENDIF
IF !oAccBal==NULL_OBJECT
	IF oAccBal:Used
		oAccBal:Close()
	ENDIF
	oAccBal:=NULL_OBJECT
ENDIF
IF !oBalYear==NULL_OBJECT
	IF oBalYear:Used
		oBalYear:Close()
	ENDIF
	oBalYear:=NULL_OBJECT
ENDIF
IF !oSys==NULL_OBJECT
	IF oSys:Used
		oSys:Close()
...
*LOCAL afsldatum AS DATE
LOCAL oWarn AS warningbox, lSuc AS LOGIC
// LOCAL oDebugBal AS DebugTransBal
// LOCAL oTrialBal AS DebugAccBal
LOCAL VWcre:={}, VWdeb:={},  VWrek:={} AS ARRAY
LOCAL VWcreF:={}, VWdebF:={},  VWCurr:={} as ARRAY  // deficit/profit foreign currency

LOCAL cTransnr,backup AS STRING
LOCAL min_saldo AS FLOAT
LOCAL i, Dep_Ptr AS INT
LOCAL pFilter as _CODEBLOCK, cFilter, cCurrency as STRING
LOCAL AfterBalance as int
Local oBord as BankOrder
local nMindate as int

IF Today() <= BalanceEndDate
   (errorbox{SELF:OWNER,'End of year not yet reached'}):Show()
   SELF:EndWindow()
   RETURN TRUE
ENDIF
* kijken of er onbestemde giften aanwezig zijn
oTrans := Transaction{}
IF !oTrans:Used
	SELF:EndWindow()
ENDIF
...
oTrans:Clearfilter()
lSuc:=oTrans:SetFilter('BFM="O".and.CRE>DEB')
oTrans:Seek(sproj)
*afsldatum:=SToD(Str(YearClose,4)+'1231')
IF !oTrans:EOF .and. oTrans:dat <=BalanceEndDate
...
* Controleer of af te sluiten jaar nog niet gerapporteerde giften bevat:
self:Pointer := Pointer{POINTERHOURGLASS}
oTrans:Clearfilter()
lSuc:=oTrans:setfilter("empty(BFM)")
IF !oTrans:OrderInfo(DBOI_NAME)=="balitemid"
	oTrans:setOrder("balitemid")
ENDIF
*oTrans:Setfilter("BFM==' '")
oMember:=Members{}
IF !oMember:Used
	SELF:EndWindow()
ENDIF
oMember:SetOrder("MBRREK")
lSuc:=oMember:setfilter("!Empty(householdid)")
oMember:gotop()
oAcc:=Account{}
IF !oAcc:Used
	SELF:EndWindow()
ENDIF
oAcc:setOrder("accid")
DO WHILE !oMember:EOF
	IF !Empty(oMember:householdid).and. oMember:GRADE#"Staf"   && medewerkers zonder HB-account overslaan
		IF oTrans:seek(oMember:accid)
   			IF oTrans:dat<= BalanceEndDate
*   			oAcc:Seek(oTrans:accid)
				oAcc:seek(oMember:accid)
			   (errorbox{SELF:OWNER,'Trnsnr '+oTrans:TransId+' with date '+DToC(oTrans:DAT);
				+' to '+AllTrim(oAcc:ACCNUMBER)+'-'+AllTrim(oAcc:Description)+' has TO be send to PMC first'}):Show()
				SELF:EndWindow()
				RETURN TRUE
			ENDIF
		ENDIF
...
if Sysparms{}:LSTREEVAL <  BalanceEndDate
	oAcc:SetFilter("!MultiCurr .and. !Currency=='"+sCURR+"'")
	oAcc:gotop()
	if !oAcc:EOF
...
d_netasset:={skap}
d_parentdep:={""}
d_depname:={sEntity+" "+AllTrim(sLand)}
d_PLdeb:={0}
d_PLcre:={0}
DO WHILE !oDep:EoF
	AAdd(d_dep,oDep:DepId)
*	AAdd(d_depnbr,AllTrim(oDep:DEPTMNTNBR))
	AAdd(d_netasset,oDep:netasset)
	AAdd(d_parentdep,oDep:ParentDep)
	AAdd(d_depname,AllTrim(oDep:Descriptn))
	AAdd(d_PLdeb,0)
	AAdd(d_PLcre,0)
	IF Empty(oDep:NetAsset)
		SELF:Pointer := Pointer{POINTERARROW}
		IF (TextBox{self:OWNER,self:oLan:WGet("Year balancing"),self:oLan:WGet('Warning: Net Asset account not defined for Department')+': '+oDep:DEPTMNTNBR+":";
		+oDep:Descriptn,BUTTONOKAYCANCEL+BOXICONHAND}):show()==BOXREPLYCANCEL
			RETURN FALSE
		ENDIF
	else
		if !oAcc:Seek(oDep:netasset)
...
self:oLan:WGet('Certificate of accountant given for year')+' '+YearBalance+'?'}
...
// * Check correctness transactions first:
// self:OWNER:STATUSMESSAGE(self:oLan:WGet("Checking correctness transactions, please wait")+"...")
// CrIndexes(Transaction{,TRUE,TRUE},,TRUE)
// CrIndexes(MBalance{,true,true},,true)
// oDebugBal:=DebugTransBal{,,,TRUE}
// IF !oDebugBal:OKButton()
// 	oDebugBal:Close()
// 	SELF:EndWindow()
// 	RETURN
// ENDIF
...
// self:OWNER:STATUSMESSAGE(self:oLan:WGet("Checking accounts are balanced, please wait")+"...")
// oAcc:Close()
// CrIndexes(Account{,TRUE,TRUE},,TRUE)
// oTrialBal:=DebugAccBal{,,,TRUE}
// IF !Empty(oTrialBal:OKButton())
// 	(InfoBox{self:OWNER,self:oLan:WGet("Year Balancing"),self:oLan:WGet("Accounts are not balanced, please correct first")}):Show()
// 	oTrialBal:Close()
// 	SELF:EndWindow()
// 	RETURN
// ENDIF
...
* Open database files exclusive:
*use accid index accid, rekoms exclusive new
oAcc:=Account{}
IF !oAcc:Used
	SELF:EndWindow()
ENDIF
oAcc:setOrder("accid")
oMBal:=MBalance{,FALSE}
IF !oMBal:Used
	SELF:EndWindow()
ENDIF
oMBal:FileLock()
oTrans:=Transaction{,FALSE}
IF !oTrans:Used
	SELF:EndWindow()
ENDIF
oTrans:FileLock()
...
IF !oLan:Used
	SELF:EndWindow()
ENDIF
oDue:=DueAmount{,FALSE}
IF !oDue:Used
	SELF:EndWindow()
ENDIF
oBord:=BankOrder{,FALSE}
IF !oBord:Used
	self:EndWindow()
ENDIF
oSys:= Sysparms{}
...
oAccBal:=AccountBalanceYear{}
IF !oAccBal:Used
	SELF:EndWindow()
ENDIF

oBalYear:=BalanceYear{}
IF !oBalYear:Used
	SELF:EndWindow()
ENDIF

* Saldi van rekeningen over af te sluiten jaar opnemen in vjsd/c van
* nieuwe jaar:
AfterBalance :=Year(BalanceEndDate+1)*100+Month(BalanceEndDate+1)


DO WHILE !oAcc:EOF 
	cCurrency:=iif(Empty(oAcc:Currency),sCurr,oAcc:Currency)

	 * balance of year to be closed = balance of previous versus following year:
// 	oMbal:GetBalance(oAcc:accid,oAcc:balitemid,LstYearClosed,BalanceEndDate,sCURR )
	oMbal:GetBalance(oAcc:accid,oAcc:balitemid,LstYearClosed,BalanceEndDate,cCurrency )
	* Indien V&W, dan total saldo bepalen voor boeken naar kapitaal en alvast
	* rekeningsaldo aanpassen:
	IF oMBal:cRubrSoort == "KO" .or. oMBal:cRubrSoort == "BA"
		* saldo van V&W verminderen met stand afgelopen jaar:
		AAdd(VWrek,oAcc:accid)
		Dep_Ptr:=AScan(d_dep,oAcc:Department)
		IF Dep_Ptr=0
			* No requested department:
			SELF:Pointer := Pointer{POINTERARROW}
			...
			RETURN FALSE
		ENDIF
		AAdd(VWdeb,oMBal:per_deb)
		AAdd(VWcre,oMBal:per_cre)
		AAdd(VWdebF,oMBal:per_debF)        // reserve place for foreign currency
		AAdd(VWcreF,oMBal:per_creF) 
		AAdd(VWCurr,cCurrency)
		IF IsNil(d_PLdeb[Dep_Ptr])
			d_PLdeb[Dep_Ptr]:=oMbal:per_deb
		ELSE
			d_PLdeb[Dep_Ptr]:=Round(d_PLdeb[Dep_Ptr]+oMbal:per_deb,DecAantal)
		ENDIF
		IF IsNil(d_PLcre[Dep_Ptr])
			d_PLcre[Dep_Ptr]:=oMbal:per_cre
		ELSE		
			d_PLcre[Dep_Ptr]:=Round(d_PLcre[Dep_Ptr]+oMbal:per_cre,DecAantal)
		ENDIF
	ELSE
   		* Bij AK/PA afletteren deb - cre jaarsaldo:
		min_saldo:=Min(oMBal:per_deb,oMBal:per_cre)
		oMBal:per_deb := Round(oMBal:per_deb - min_saldo,DecAantal)
		oMBal:per_cre := Round(oMBal:per_cre - min_saldo,DecAantal)
		min_saldo:=Min(oMBal:per_debF,oMBal:per_creF)
		oMBal:per_debF := Round(oMBal:per_debF - min_saldo,DecAantal)
		oMBal:per_creF := Round(oMBal:per_creF - min_saldo,DecAantal)
	ENDIF

	IF oMBal:per_deb#0.or.oMBal:per_cre#0 .or. oMBal:per_debF # 0 .or. oMBal:per_creF # 0
	* per_deb en per_cre bewaren in AccountBalanceYear van volgende jaar:
		oAccBal:seek(oAcc:accid+Str(AfterBalance,6)+sCURR)
		IF oAccBal:EOF
			oAccBal:Append()
			oAccBal:accid := oAcc:accid
			oAccBal:YearStart := AfterBalance/100
		 	oAccBal:MonthStart := AfterBalance%100 
		 	oAccBal:CURRENCY:=sCurr
		ENDIF
		oAccBal:svjd := oMBal:per_deb
		oAccBal:svjc := oMBal:per_cre
		if oAcc:Currency # sCURR
			oAccBal:seek(oAcc:accid+Str(AfterBalance,6)+oAcc:Currency)
			IF	oAccBal:EOF
				oAccBal:Append()
				oAccBal:accid	:=	oAcc:accid
				oAccBal:YearStart	:=	AfterBalance/100
				oAccBal:MonthStart := AfterBalance%100	
				oAccBal:Currency:=oAcc:Currency
			ENDIF			
			oAccBal:svjd := oMBal:per_debF
			oAccBal:svjc := oMBal:per_creF
		endif
	ENDIF
// 	if !cCurrency==sCurr
...
// 		IF oMBal:cRubrSoort == "KO" .or. oMBal:cRubrSoort == "BA"
// 			i:=Len(VWCurr)				
// 			VWdebF[i]:=oMbal:per_debF
// 			VWcreF[i]:=oMbal:per_creF
// 		else
// 			min_saldo:=Min(oMbal:per_debF,oMbal:per_creF)
// 			oMBal:per_debF := Round(oMBal:per_debF - min_saldo,DecAantal)
// 			oMBal:per_creF := Round(oMBal:per_creF - min_saldo,DecAantal)
...
// 			* per_deb en per_cre bewaren in AccountBalanceYear van volgende jaar:
// 			oAccBal:Seek(oAcc:accid+Str(AfterBalance,6))+cCurrency
// 			IF oAccBal:EOF
// 				oAccBal:Append()
// 				oAccBal:accid := oAcc:accid
// 				oAccBal:YearStart := AfterBalance/100
// 			 	oAccBal:MonthStart := AfterBalance%100 
// 			 	oAccBal:CURRENCY:=cCurrency
// 			ENDIF
// 			oAccBal:svjd := oMbal:per_debF
// 			oAccBal:svjc := oMbal:per_creF
// 		endif
...
* Maak boekingen aan voor wegboeken van V&W-saldi naar kapitaal:
cTransnr := TransKey{}:KeyValue

FOR i = 1 TO Len(VWrek)
	IF VWdeb[i]#0 .or. VWcre[i]#0
		oTrans:append()
		oTrans:docid := 'CL'+StrZero(YearClose%100,2)+StrZero(MonthClose,2)
		oTrans:accid := VWrek[i]
		oTrans:description := oLan:RGet('Closing year',,"!")+' '+YearBalance
		oTrans:deb := -VWdeb[i]
		oTrans:cre := -VWcre[i]
		oTrans:CURRENCY:=VWCurr[i]
		oTrans:DEBFORGN:=-VWdebF[i]
		oTrans:CREFORGN:=-VWcreF[i] 
		oTrans:Currency:=VWCurr[i]
		oTrans:dat := BalanceEndDate
		oTrans:USERID := LOGON_EMP_ID
		oTrans:TransId := cTransnr
	ENDIF
NEXT
* Make records to netassetaccounts per department:
self:SubDepartment(1,cTransnr,AfterBalance)


* Wijzig afgesloten jaar:
oBalYear:Append()
oBalYear:YEARSTART:=Year(LstYearClosed)
oBalYear:MONTHSTART:=Month(LstYearClosed)
oBalYear:YEARLENGTH:=YearClose*12+MonthClose+1-Year(LstYearClosed)*12-Month(LstYearClosed)
oBalYear:STATE:="C"
oBalYear:Commit()
LstyearClosed	:=BalanceEndDate+1

oSys:GoTO(1)
oSys:Reclock()
oSys:yearclosed := YearClose
if oSys:MinDate < LstyearClosed 
	oSys:MinDate := LstyearClosed
	MinDate:=LstyearClosed 
endif
oSys:commit()
oSys:unLock()
oSys:Close()
...
* Schoning:
* Openstaande posten schonen:

cFilter:='(DToS(invoicedate)<"'+DToS(Today()-120)+'".and.AmountRecvd >=AmountInvoice).or.Year(invoicedate)<'+Str(Year(Today())-1,4,0)
pFilter:=&("{||"+ cFilter+"}")
lSuc:=oDue:SetFilter(pFilter,cFilter)
oDue:Gotop()
IF !oDue:EOF
...
cFilter:='!empty(datepayed).and.(DToS(DATEPAYED)<"'+DToS(Today()-200)+'")'
pFilter:=&("{||"+ cFilter+"}")
lSuc:=oBord:SetFilter(pFilter,cFilter)
oBord:gotop()
IF !oBord:EOF
	self:STATUSMESSAGE(self:oLan:WGet("Removing of payable amounts, moment please"))
	lSuc:=oBord:FileLock()
	lSuc:=oBord:DeleteAll()
	lSuc:=oBord:pack()
ENDIF
oBord:Close()
...
* Maandsaldi schonen:
/*oMBal:ClearFilter()
lSuc:= oMBal:SetFilter("YEAR<="+Str(YearStart-9,4,0))
oMBal:Gotop()
IF !oMBal:EOF
   backup:='mnds'+Str(YearStart-9,4)
	SELF:StatusMessage( ;
   "Archiving of month balance values older than 10 years to file "+backup+", moment please")
   lSuc:=oMBal:copyDB(backup)
	SELF:StatusMessage("Removing of month balance values, moment please")
   oMBal:GoTop()
   lSuc:=oMBal:DeleteAll()
   lSuc:=oMBal:Pack()
ENDIF            */
oMBal:Commit()
oMBal:Close()    
...
* Mutaties schonen:
lSuc:=oTrans:ClearIndex()
lSuc:= oTrans:ClearFilter()
lSuc:= oTrans:SetFilter('DTOS(dat)<="'+DToS(BalanceEndDate)+'"')
oTrans:GoTop()
backup:='Tr'+Str(YearStart,4)+StrZero(MonthStart,2)
self:STATUSMESSAGE(self:oLan:WGet("Archiving of transactions to file")+" "+backup+", "+self:oLan:WGet("moment please"))
oTrans:CopyDB(backup)
self:STATUSMESSAGE(self:oLan:WGet("Removing of transactions, moment please"))
lSuc:=oTrans:Gotop()
lSuc:=oTrans:DeleteAll()
*
lSuc:=oTrans:SetIndex()
oTrans:Unlock()
oTrans:Close()
oTrans:=NULL_OBJECT
self:STATUSMESSAGE(self:oLan:WGet("Compacting other data, moment please"))
...
self:SetTexts()
	oMBal:=MBalance{}
	oMbal:GetBalYear(Year(LstYearClosed),Month(LstYearClosed))
	YearStart:=oMbal:YEARSTART
	MonthStart:=oMBal:MONTHSTART
	YearClose :=oMBal:YEAREND
	MonthClose:=oMBal:MONTHEND
	BalanceYearEnd:=YearClose*12+MonthClose
	BalanceEndDate:=SToD(Str(YearClose,4)+StrZero(MonthClose,2)+StrZero(MonthEnd(MonthClose,YearClose),2))
	oDCStartYearText:TextValue:=Str(YearStart,4)+":"+StrZero(MonthStart,2)+"  -  "+;
	Str(YearClose,4)+":"+StrZero(MonthClose,2)
*	oDCEndYearText:TextValue:=Str(YearClose,4)+"  "+Str(MonthClose,2)
	YearBalance := oDCStartYearText:TextValue
	oMBal:Close()
	RETURN NIL
METHOD SubDepartment(p_depptr as int, cTransnr as string,AfterBalance as int) as void pascal CLASS YearClosing
* Recursive processing of a department with its subdeparments
*
LOCAL subDepPtr AS INT
LOCAL min_saldo, VW_totdeb, VW_totcre as FLOAT
...
DO WHILE TRUE
	...
	IF Empty(subDepPtr)
		EXIT
	ELSE
		SELF:SubDepartment(subDepPtr,cTransnr,AfterBalance)
		* consolidate values of subdepartment into itself:
...
IF !Empty(d_netasset[p_depptr])
	VW_totdeb:=d_PLdeb[p_depptr]
	VW_totcre:=d_PLcre[p_depptr]
	min_saldo:=Min(VW_totdeb,VW_totcre)   && afletteren deb - cre
	VW_totdeb := Round(VW_totdeb - min_saldo,DecAantal)
	VW_totcre := Round(VW_totcre - min_saldo,DecAantal)
	oTrans:Append()
	oTrans:docid := 'CL'+YearBalance
	oTrans:accid :=d_netasset[p_depptr]
	oTrans:description := oLan:Get('Closing year',,"!")+' '+oDCStartYearText:TEXTvalue
	oTrans:deb := VW_totdeb
	oTrans:cre := VW_totcre
	oTrans:DEBFORGN := VW_totdeb
	oTrans:CREFORGN := VW_totcre
	oTrans:dat := BalanceEndDate
	oTrans:USERID := LOGON_EMP_ID
	oTrans:TransId := cTransnr
	oTrans:Currency := sCURR
	* adapt last year balance of net asset account:
	oAccBal:Seek(d_netasset[p_depptr]+Str(AfterBalance,6)+sCURR)
	IF oAccBal:EOF
		oAccBal:Append()
		oAccBal:accid := d_netasset[p_depptr]
		oAccBal:YearStart := AfterBalance/100
	 	oAccBal:MonthStart := AfterBalance%100 
	 	oAccBal:Currency:=sCURR
	ENDIF
	oAccBal:svjd := Round(oAccBal:svjd + VW_totdeb,decaantal)
	oAccBal:svjc := Round(oAccBal:svjc + VW_totcre,decaantal)

/*	* maandsaldo van kapitaal aanpassen: (first month IN NEXT balance year)
	oMBal:ChgBalance(d_netasset[p_depptr],BalanceEndDate+1,VW_totdeb,VW_totcre)   */
	* Clear d_PLdeb/cre:
	d_PLdeb[p_depptr]:=0
	d_PLcre[p_depptr]:=0
ENDIF
